...
export const HeatmapChart = React.memo(function HeatmapChart({ posts }: Props) {
  ...
  const grid = React.useMemo(() => {
    ...
    posts.forEach(post => {
      ...
      newGrid[day][hour].posts.push(post);
      // Calculate weighted score (75% upvotes, 25% comments)
      newGrid[day][hour].score += (post.score * 0.75 + post.num_comments * 0.25);
    });
  ...
  const maxScore = React.useMemo(() => 
    Math.max(...grid.flatMap(row => row.map(cell => cell.score))), 
    [grid]
  );

  // Calculate cell intensity with extended falloff effect
  const getCellIntensity = React.useCallback((day: number, hour: number) => {
  ...
  const getCellBackground = React.useCallback((intensity: number, isHovered: boolean = false) => {
  ...
  const getCellGlow = React.useCallback((intensity: number, isHovered: boolean = false) => {
  ...
  const getNeighborCoords = React.useCallback((day: number, hour: number) => {
  ...
  const getTopPosts = React.useCallback((posts: Post[]) => {
    return [...posts]
  ...
  const formatTime = React.useCallback((hour: number) => {
  ...
  const getTooltipPosition = React.useCallback((rect: DOMRect, dayIndex: number, hour: number) => {
    ...
    } else if (x - tooltipWidth/2 < edgePadding) {
      x = rect.left;
    } else {
  ...
  // Cleanup timeouts and DOM elements on unmount
  useEffect(() => {
    return () => {
  ...
  const showTooltip = React.useCallback((cell: HTMLElement, posts: Post[], dayIndex: number, hour: number) => {
    if (tooltipTimeoutRef.current) {
      window.clearTimeout(tooltipTimeoutRef.current);
    }
                ...
                >
                  <div
                    ...
                    onMouseEnter={(e) => {
                      ...
                      // Add hover effect to neighboring cells with flat aesthetic using more efficient approach
                      const neighbors = getNeighborCoords(dayIndex, hour);
                      const cellsToUpdate: Array<[HTMLElement, number]> = [];
                      
                      // First, gather all elements to update
                      neighbors.forEach(([d, h]) => {
                        // Use data attributes to find cells more efficiently
                        const selector = `[data-day="${d}"][data-hour="${h}"]`;
                        const neighborCell = document.querySelector(selector) as HTMLElement | null;
                        if (neighborCell) {
                          const distance = Math.sqrt(
                            Math.pow(d - dayIndex, 2) + Math.pow(h - hour, 2)
                          );
                          cellsToUpdate.push([neighborCell, distance]);
                        }
                      });
                      
                      // Then, batch the DOM updates
                      if (cellsToUpdate.length > 0) {
                        // Use requestAnimationFrame for smoother rendering
                        requestAnimationFrame(() => {
                          cellsToUpdate.forEach(([neighborCell, distance]) => {
                            const falloff = Math.exp(-distance * 1.2); // Steeper falloff for cleaner edges
                            neighborCell.classList.add('neighbor-hover');
                            neighborCell.style.opacity = `${1 - falloff * 0.1}`; // Subtle opacity change
                            neighborCell.style.backgroundColor = interpolateColor(
                              Math.min(getCellIntensity(
                                parseInt(neighborCell.getAttribute('data-day') || '0'), 
                                parseInt(neighborCell.getAttribute('data-hour') || '0')
                              ) + falloff * 0.15, 1)
                            );
                            neighborCell.style.borderColor = `rgba(76, 175, 80, ${falloff * 0.2})`; // Subtle border highlight
                          });
                        });
                      }
                    }}
